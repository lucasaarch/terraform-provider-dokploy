...
- `health_check` (Block, Optional) Docker Swarm healthcheck. Omit to skip. (see [below for nested schema](#nestedblock--health_check))
...
- `replicas` (Number) Number of Docker Swarm replicas. Defaults to whatever Dokploy uses (typically 1).
- `restart_policy` (Block, Optional) Docker Swarm restart policy. Omit to skip. (see [below for nested schema](#nestedblock--restart_policy))
...
- `test` (List of String) Command (e.g. `["CMD", "curl", "-f", "http://localhost/health"]`).
- `timeout` (String) Probe timeout.


<a id="nestedblock--restart_policy"></a>
### Nested Schema for `restart_policy`

Optional:

- `condition` (String) `none`, `on-failure`, or `any`.
- `delay` (String) Delay between attempts (e.g. `5s`).
- `max_attempts` (Number) Maximum restart attempts.
- `window` (String) Evaluation window for max attempts.
